                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a
                                href="{{ route('admin.api-keys.show', $apiKey) }}">{{ $apiKey->name }}</a></li>
                ...
                <form action="{{ route('admin.api-keys.update', $apiKey) }}" method="POST" id="edit-api-key-form">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        @enderror
                                        <small class="form-text text-muted">A descriptive name to identify this API
                                            key</small>
                                    </div>
                                </div>
                                <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                    rows="3" placeholder="Describe the purpose and usage of this API key">{{ old('description', $apiKey->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="number"
                                            class="form-control mt-2 @error('rate_limit_per_minute') is-invalid @enderror"
                                            id="custom_rate_limit" name="custom_rate_limit" min="1" max="10000"
                            ...
                            <!-- แก้ไขส่วน permissions ใน edit.blade.php -->

                            <div class="form-group">
                                <label class="required">Permissions</label>
                                <div class="row">
                                    @php
                                        $currentPermissions = old(
                                            'permissions',
                                            $apiKey->apiPermissions->pluck('name')->toArray(),
                                        );
                                    @endphp
                                        ...
                                        <div class="col-md-6 mb-2">
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input"
                                                    id="permission_{{ $loop->index }}" name="permissions[]"
                                                    value="{{ $permission }}"
                                ...
                                <div class="mt-2">
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-secondary"
                                        id="select-none-permissions">
                                        Select None
                                    </button>
                                    <button type="button" class="btn btn-sm btn-outline-info"
                                        id="select-common-permissions">
                                        Select Common
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('ip_whitelist') is-invalid @enderror" id="ip_whitelist" name="ip_whitelist"
                                    rows="3" placeholder="Enter IP addresses separated by commas (e.g., 192.168.1.1, 10.0.0.0/24)">{{ old('ip_whitelist', $apiKey->ip_whitelist ? implode(', ', $apiKey->ip_whitelist) : '') }}</textarea>
                                @error('ip_whitelist')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="form-text text-muted">Leave empty to allow all IP addresses. Supports CIDR
                                    notation.</small>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input" id="auto_notifications"
                                            name="auto_notifications" value="1"
                                    ...
                                    <small class="form-text text-muted">
                                        Automatically send notifications about API key events (regeneration, status changes,
                                        etc.)
                                    </small>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="url"
                                            class="form-control @error('notification_webhook') is-invalid @enderror"
                            ...
                            @if ($apiKey->usage_count > 0)
                                <div class="alert alert-warning mt-3">
                                    <small>
                                        ...
                                        <strong>Note:</strong> This API key has been used
                                        {{ number_format($apiKey->usage_count) }} times.
                                        Changes to permissions or rate limits will take effect immediately and may impact
                                ...
                                <div class="timeline-content">
                                    ...
                                    <p class="timeline-text">
                                        ...
                                        <br><small class="text-muted">by
                                            {{ $apiKey->createdBy->display_name ?? 'System' }}</small>
                                    </p>
                                </div>
                            </div>
                                ...
                                <div class="timeline-item">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <p class="timeline-text">
                                            ...
                                            <br><small class="text-muted">by
                                                {{ $apiKey->regeneratedBy->display_name ?? 'System' }}</small>
                                        </p>
                                    </div>
                                </div>
                            @endif
                                ...
                                <div class="timeline-item">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <p class="timeline-text">
                                            ...
                                            <br><small class="text-muted">by
                                                {{ $apiKey->statusChangedBy->display_name ?? 'System' }}</small>
                                        </p>
                                    </div>
                                </div>
                            @endif
                                ...
                                <div class="timeline-item">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <p class="timeline-text">
                                            ...
                                            <br><small class="text-muted">by
                                                {{ $apiKey->usageResetBy->display_name ?? 'System' }}</small>
                                        </p>
                                    </div>
                                </div>
                            @endif
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <div class="list-group list-group-flush">
                            <form action="{{ route('admin.api-keys.toggle-status', $apiKey) }}" method="POST"
                                class="d-inline">
                                @csrf
                                <button type="submit" class="list-group-item list-group-item-action"
                                    onclick="return confirm('Are you sure you want to {{ $apiKey->is_active ? 'deactivate' : 'activate' }} this API key?')">
                                    <i
                                        class="fas fa-{{ $apiKey->is_active ? 'pause' : 'play' }} text-{{ $apiKey->is_active ? 'warning' : 'success' }}"></i>
                                    <span class="ml-2">{{ $apiKey->is_active ? 'Deactivate' : 'Activate' }} API
                                        Key</span>
                                </button>
                            </form>

                            <form action="{{ route('admin.api-keys.reset-usage', $apiKey) }}" method="POST"
                                class="d-inline">
                                @csrf
                                <button type="submit" class="list-group-item list-group-item-action"
    ...
    <script>
        $(document).ready(function() {
            ...
            function showUsageWarning() {
                ...
                const permissionsChanged = JSON.stringify(originalPermissions.sort()) !== JSON.stringify(
